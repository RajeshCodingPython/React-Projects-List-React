...
class App extends Component {
  state = {avtiveTabID: tabsList[0].tabId}

  getFilterProjects = () => {
    const {avtiveTabID} = this.state
    const fitetProjects = projectsList.filter(
      eachProjects => eachProjects.category === avtiveTabID,
    )
    return fitetProjects
  }

  onChngeElment = tabId => {
    this.setState({avtiveTabID: tabId})
  }
        ...
        <ul className="tabs-container">
          {tabsList.map(tabDetails => (
            <TabItem
              ...
              isActive={avtiveTabID === tabDetails.tabId}
            />
          ))}
        </ul>
